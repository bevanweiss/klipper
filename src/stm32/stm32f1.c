...
lookup_clock_line(uint32_t periph_base)
{
    ...
    if (periph_base == SDIO_BASE) {
        return (struct cline){.en=&RCC->AHBENR, .bit=1 << 10};
    }
#endif
#ifdef FSMC_R_BASE
    if (periph_base == FSMC_R_BASE) {
        return (struct cline){.en=&RCC->AHBENR, .bit=1 << 8};
    }
#endif
...
gpio_peripheral(uint32_t gpio, uint32_t mode, int pullup)
{
    ...
    if (mode == GPIO_INPUT) {
        ...
        // so only support AF_PP or AF_OD here
        if(!(mode & (GPIO_OUTPUT | GPIO_OPEN_DRAIN))) {
            cfg = 0x8;
        } else {
